...
//            默认是classPath路径
    ...
    public static UserVo getLoginUser(HttpServletRequest request) {
        ...
        return (UserVo) session.getAttribute(Const.LOGIN_SESSION_KEY);
    }
    ...
    public static Integer getCookieUid(HttpServletRequest request) {
        if (null != request) {
            Cookie cookie = cookieRaw(Const.USER_IN_COOKIE, request);
            if (cookie != null && cookie.getValue() != null) {
                try {
                    String uid = Tools.deAes(cookie.getValue(), Const.AES_SALT);
    ...
    public static void setCookie(HttpServletResponse response, Integer uid) {
        try {
            String val = Tools.enAes(uid.toString(), Const.AES_SALT);
            boolean isSSL = false;
            Cookie cookie = new Cookie(Const.USER_IN_COOKIE, val);
            cookie.setPath("/");
